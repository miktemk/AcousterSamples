...
class Py2XML():
	...
	def parse( self, pythonObj, objName=None ):
		...
		if isinstance( pythonObj, dict ):
			...
			self.data = "<%(n)s>%(o)s</%(n)s>\n" % { 'n':objName, 'o':str( pythonObj ) }
	...
	def _PyDict2XML( self, pyDictObj, objName=None ):
		...
		tagStr += INDENT * self.indent
		if childStr == "":
			tagStr += "<%(n)s%(a)s />" % { 'n':objName, 'a':attrStr }
		else:
			tagStr += "<%(n)s%(a)s>\n%(c)s" % { 'n':objName, 'a':attrStr, 'c':childStr }
			tagStr += INDENT * self.indent
			tagStr += "</%(n)s>" % { 'n':objName, 'a':attrStr, 'c':childStr }
		tagStr += '\n'
		self.indent -= 1
	...
	def _PyList2XML( self, pyListObj, objName=None ):
		...
		tagStr += INDENT * self.indent
		if childStr == "":
			tagStr += "<%(n)s></%(n)s>" % { 'n':objName, 'c':childStr }
			#tagStr += "<%(n)s%(a)s />" % { 'n':objName, 'a':attrStr }
		else:
			tagStr += "<%(n)s>\n%(c)s" % { 'n':objName, 'c':childStr }
			tagStr += INDENT * self.indent
			tagStr += "</%(n)s>" % { 'n':objName, 'c':childStr }
